...
label(MIDX, 8.77, "9-15 calls", "#0C6457", size=7.2, weight="normal")

arrow((X1 + BW, 7.30), (X2, Y_EDIT + 0.72))
label(MIDX, 7.55, "E · revise", "#8E2748")
label(MIDX, 7.27, "3 calls", "#8E2748", size=7.2, weight="normal")

# F re-enters the planner rather than starting a new trip, so it shares C's edge into the
# ReAct Planner and is labelled beneath it. It is a route, not a module: every step it
# logs carries one of the module names already boxed above.
label(MIDX, 8.35, "F · resume", "#0C6457", size=8)
label(MIDX, 8.09, "4-5 calls", "#0C6457", size=7.2, weight="normal")
...
ax.text(0.7, 2.66, "Validation & Audit  (schemas.py · audit.py)", fontsize=9.5,
        fontweight="bold", color=INK, ha="left", va="center", bbox=_plate)
ax.text(6.5, 2.66, "profile · draft plan · verdict · budget ceiling · schedule · hours   —   "
...
ax.text(0.7, 1.76, "Supabase — conversation state", fontsize=9.5,
...
ax.text(0.7, 0.90, "Optional — unconfigured, the agent runs exactly as before.",
...
ax.text(8.25, 1.46, "maps · search · reviews · weather  (real APIs)", fontsize=7.2,
        color=MUTED, ha="left")
ax.text(8.25, 1.24, "flight_search · booking  (fictive, no purchase)", fontsize=7.2,
        color=MUTED, ha="left")
...
ax.text(8.0, 0.68, "Every LLM call is time-gated and counted: the run stops before it can "
                   "exceed the function\nbudget or the per-run call ceiling, and degrades to a "
